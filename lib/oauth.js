...
export async function requestDeviceCode(pkceChallenge) {
  console.log('[OAuth] 请求设备授权码, challenge:', pkceChallenge.substring(0, 10) + '...')
  try {
    ...
    if (err.response) console.error('[OAuth] 响应状态:', err.response.status, JSON.stringify(err.response.data))
    throw err
  }
}
