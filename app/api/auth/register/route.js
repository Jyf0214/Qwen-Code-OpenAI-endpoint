...
export async function POST(request) {
  try {
    ...
    if (username.length < 2 || username.length > 50) return NextResponse.json({ success: false, message: '用户名长度需在 2-50 个字符之间' }, { status: 400 })
    if (password.length < 6) return NextResponse.json({ success: false, message: '密码长度至少 6 个字符' }, { status: 400 })
    const userCount = await prisma.user.count()
    if (userCount > 0) return NextResponse.json({ success: false, message: '已有管理员账户，请使用登录页面' }, { status: 403 })
    const existingUser = await prisma.user.findUnique({ where: { username } })
    if (existingUser) return NextResponse.json({ success: false, message: '用户名已被占用' }, { status: 409 })
