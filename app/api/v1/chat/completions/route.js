...
const API_CONFIG = { baseUrl: 'https://portal.qwen.ai', timeout: 60000 }

export async function POST(request) {
  try {
    const body = await request.json()
    const account = await Account.getLeastUsedAccount()
    if (!account) return NextResponse.json({ error: { message: '没有可用的账号', type: 'no_available_account', code: 503 } }, { status: 503 })
    if (!account.accessToken || !account.expiresAt || new Date(account.expiresAt) <= new Date()) return NextResponse.json({ error: { message: `账号 ${account.name} 的 token 已过期`, type: 'token_expired', code: 401 } }, { status: 401 })
    ...
    return NextResponse.json({ ...response.data, usage: usage || response.data?.usage || { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 } })
  } catch (error) {
    return NextResponse.json({ error: { message: error.message, type: 'api_error', code: 500 } }, { status: 500 })
  }
}
